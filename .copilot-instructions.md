...
> **Privacy & Security**
> - **NEVER commit real IP addresses** - use placeholder IPs (e.g., 192.0.2.10, 198.51.100.20)
> - **NEVER commit real names or PII** - use placeholder names (e.g., alice, bob, user1)
> - **NEVER commit real speaker profile names** - use generic examples (e.g., alice_fr, bob_en)
> - Obfuscate any real configuration values in documentation, code examples, and commit messages
...
- `record_samples.py` and `enroll.py` are **client-side tools** stored in the Aria repo under client tooling (e.g., `client/macos/`).
- They are not part of the server runtime and must not be imported by server code.
...
ARIA_SPK_HOT_RELOAD=0
ARIA_SPK_DEBUG_DUMP_WAV=0
ARIA_SPK_SELF_NAMES=aria_fr,aria_en
```
...
## PART 4 — PLUGIN SYSTEM FOR TOOL CALLING (HOME AUTOMATION)

### Overview

ARIA implements a **plugin-based architecture** that enables the LLM to interact with external systems (e.g., Jeedom home automation) through **function calling** (tool use).

The plugin system is modular and can be fully disabled via environment variables.

**Documentation:** See `docs/PLUGIN_SYSTEM.md` for comprehensive architecture details.
...
**Plugin components:**
1. Plugin Registry (`app/plugins/registry.py`) — Plugin lifecycle management
2. Inventory Provider (`app/plugins/inventory/`) — Device discovery via MQTT
3. Actions Provider (`app/plugins/actions/`) — Command execution via MQTT
4. LLM Tools (`app/llm/tools.py`) — OpenAI-compatible function schemas

**Tool calling flow:**
```
User: "allume la lumière du salon"
  → LLM calls find_devices(query="lumière salon")
  → Returns candidates with cmd_id
  → LLM calls execute_command(cmd_id="5587")
  → MQTT publish to jeedom/cmd/set/5587
  → LLM confirms: "J'ai allumé la lumière du salon."
```
...
export ARIA_LLM_MODEL=qwen2.5:7b-instruct  # 7b recommended for reliable tool calling
export ARIA_LLM_NUM_PREDICT=1024
```

**Jeedom MQTT2 Payload Formats:**
- Binary (ON/OFF): `{}`
- Slider (0-99): `{"slider": X}`
- Select: `{"select": "value"}`
- Message: `{"title": "...", "message": "..."}`
- Color: `{"color": "#rrggbb"}`

**System prompt requirements:**
- MUST instruct LLM to always use tools for device control
- MUST NOT allow LLM to claim control without calling execute_command
- MUST enforce workflow: find_devices() → execute_command() → confirm

**Testing:**
```bash
# Health check
curl http://localhost:8000/healthz | jq '.plugins'

# Tool calling
curl -X POST http://localhost:8000/api/llm/process \
  -H "Content-Type: application/json" \
  -d '{"text": "allume la lumière", "use_tools": true}' | jq
```
